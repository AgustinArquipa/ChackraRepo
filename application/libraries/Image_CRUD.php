...
class image_CRUD {
	...
	protected function _create_thumbnail($image_path, $thumbnail_path, $ancho = '', $alto = '')
	{
        if ($ancho != '' && $alto != ''){
            $this->image_moo
			->load($image_path)
			->resize_crop($ancho,$alto)
		...
		if(!empty($state_info))
		{
			switch ($state_info->name) {
				...
				case 'upload_file':
					...
					if ($file_name !== false) {
						$this->_create_thumbnail( $this->image_path.'/'.$file_name , $this->image_path.'/'.$this->thumbnail_prefix.'_540_400_'.$file_name, 540, 400);
                        $this->_create_thumbnail( $this->image_path.'/'.$file_name , $this->image_path.'/'.$this->thumbnail_prefix.'_247_185_'.$file_name, 247, 185);
                        $this->_create_thumbnail( $this->image_path.'/'.$file_name , $this->image_path.'/'.$this->thumbnail_prefix.'_150_113_'.$file_name, 150, 113);
