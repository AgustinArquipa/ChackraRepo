...
class TCPDF {
	 ...
	 * @public
	 */
	public function setHeaderData($ln='', $lw=0, $ht='', $hs='', $tc=array(0,0,0), $lc=array(0,0,0), $log_derecha) {
		$this->header_logo = $ln;
        ...
        $this->header_logo_derecha = $log_derecha;
	}
	...
	public function getHeaderData() {
        ...
        $ret['logo_derecha'] = $this->header_logo_derecha;
		return $ret;
	}
...
class MYPDF extends TCPDF {
    ...
    public function Header() {
        ...
        if ($this->header_xobjid === false) {
            ...
            if ($headerdata['logo_derecha'] != ''){
                $this->Image($headerdata['logo_derecha'], 170, 0, 20, '', 'JPG', '', '', false, 300, '', false, false, 0);       
            }            
            //$pdf->Image('assets/uploads/inspecciones/m_583a20542491c.jpg', 10, 2, 25, '', 'JPG', '', '', false, 300, 'R', false, false, 0,false, false, false);
			...
			$this->SetFont($headerfont[0], 'B', $headerfont[2] + 1);
			$this->SetX($header_x);
			$this->Cell($cw-30, $cell_height, $headerdata['title'], 0, 1, 'C', 0, '', 0);
			// header string
			$this->SetFont($headerfont[0], $headerfont[1], $headerfont[2]);
			$this->SetX($header_x);
			$this->Cell($cw-30, $cell_height, $headerdata['string'], 0, 1, 'C', 0, '', 0);
			// print an ending header line
			$this->SetLineStyle(array('width' => 0.85 / $this->k, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => $headerdata['line_color']));
			$this->SetY((2.835 / $this->k) + max($imgy, $this->y));
			if ($this->rtl) {
